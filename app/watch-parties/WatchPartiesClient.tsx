...
export default function WatchPartiesClient({ parties }: { parties: WatchParty[] }) {
  ...
  // Merge official + local parties for display
  const allParties: (WatchParty | (LocalWatchParty & { _local: true }))[] = useMemo(() => {
    const localAsWatchParty = localParties.map((lp) => ({ ...lp, _local: true as const }));
    return [...parties, ...localAsWatchParty];
